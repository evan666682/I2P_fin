...
SlowBullet::SlowBullet(Engine::Point position, Engine::Point forwardDirection, float rotation, Turret* parent,float slowFactor, float slowDuration) :
	Bullet("play/bullet-5.png", 100, 2, position, forwardDirection, rotation + ALLEGRO_PI / 2, parent),slowFactor(slowFactor), slowDuration(slowDuration){
}

void SlowBullet::Update(float deltaTime) {
	if (!Target) {
...
void SlowBullet::OnExplode(Enemy* enemy) {
	...
	enemy->ApplySlow(slowFactor, slowDuration);  // slow-down enemy
}
