...
#include "Turret/TypeCTurret.hpp"
...
void PlayScene::OnMouseUp(int button, int mx, int my) {
	...
	const int y = my / BlockSize;
	if (button &1) {
		if (mapState[y][x] == TILE_FLOOR ||(MapId==2)) {
			if (!preview)
				return;
			// Check if valid.
...
void PlayScene::ConstructUI() {
	...
	UIGroup->AddNewObject(UIScore = new Engine::Label(std::string("Score ") + std::to_string(scores), "pirulen.ttf", 24, 1294, 128));
	TurretButton* btn;
	// Button 1
	...
	btn = new TurretButton("play/floor.png", "play/dirt.png",
		...
		Engine::Sprite("play/turret-6.png", 1294, 252 - 8, 0, 0, 0, 0)
		, 1294, 252, LaserTurret::Price);
...
void PlayScene::UIBtnClicked(int id) {
	...
	else if (id == 3 && money >= TypeCTurret::Price)
		preview = new TypeCTurret(0, 0);
	if (!preview)
		return;
	preview->Position = Engine::GameEngine::GetInstance().GetMousePosition();
	...
	while (!que.empty()) {
		...
		//               mapState[y][x] is TILE_DIRT if it is empty.
		 for (const auto& dir : directions) {
            ...
            // Check if the new position is within bounds and is TILE_DIRT.
            if (newX >= 0 && newX < MapWidth && newY >= 0 && newY < MapHeight && 
                mapState[newY][newX] == TILE_DIRT && map[newY][newX] == -1) {
                
                // Update the distance and push the new position to the queue.
...
void PlayScene::UpdateEnemyPath(Enemy* enemy) {
    // 重新计算路径
    enemy->UpdatePath(mapDistance);
}